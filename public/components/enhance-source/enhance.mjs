const { parse, fragment, serialize } = parse5
import isCustomElement from '/components/enhance-source/lib/is-custom-element.mjs'
import { encode, decode } from '/components/enhance-source/lib/transcode.mjs'
// const { parse, fragment: parseFragment, serialize } = parse5

export default function Enhancer(options = {}) {
  const { initialState = {}, elements } = options
  ...
  return function html(strings, ...values) {
    ...
    const templates = fragment(
      moduleNames.map((name) => template(name, elements)).join('')
    )
...
function expandTemplate(node, elements, store) {
  const frag = fragment(
    renderTemplate(node.tagName, elements, node.attrs, store) || ''
  )
...
function findSlots(node) {
  ...
  const find = (node) => {
    for (const child of node.childNodes) {
      ...
      if (!isCustomElement(child.tagName) && child.childNodes) {
        find(child)
      }
    }
  }
